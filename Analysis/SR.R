...
for(i in 1 : nrow(st.list.name)){
  ...
  for(j in 1 : length(st_season_list)){
    ...
    mean_yyyy_season <- mean(st_season_list[[j]] $ SR, na.rm = T)
    ...
    for(k in 1980 : 2018){
      ...
      mean_yyyy_c[k - 1979] <- mean(st_yyyy $ SR, na.rm = T)
    }
...
ggplot() +  
  ...
  xlab("year") + ylab("SR") + ggtitle("Summer yearly change of SR of each station for 1980-2018") +
  theme()
