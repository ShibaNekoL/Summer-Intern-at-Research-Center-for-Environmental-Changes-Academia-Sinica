...
for(i in 1 : nrow(st.list.name)){
  ...
  for(j in 1 : length(st_season_list)){
    ...
    mean_yyyy_season <- mean(st_season_list[[j]] $ Ta, na.rm = T)
    ...
    for(k in 1980 : 2018){
      ...
      mean_yyyy_c[k - 1979] <- mean(st_yyyy $ Ta, na.rm = T)
    }
...
ggplot() +  
  ...
  geom_line(data = st_seasonframe_d_mean, aes(x = yyyy, y = winter, colour = stname, group = stname), size = 1) + 
  geom_point(data = st_seasonframe_d_mean, aes(x = yyyy, y = winter, colour = stname,group = stname), size = 2) +
